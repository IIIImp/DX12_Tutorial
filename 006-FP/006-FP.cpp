...
private:
	XMVECTOR EyePosition = XMVectorSet(4, 3, 4, 1);
	XMVECTOR FocusPosition = XMVectorSet(0, 1, 1, 1);
	XMVECTOR UpDirection = XMVectorSet(0, 1, 0, 0);

	XMVECTOR ViewDirection = {};// = XMVector3Normalize(FocusPosition - EyePosition);
	float FocalLength = 0.0f; // = XMVectorGetX(XMVector3Length(FocusPosition - EyePosition));
...
void Camera::Walk(float Stride)
{
	EyePosition += Stride * ViewDirection;
	FocusPosition += Stride * ViewDirection;
}

void Camera::Strafe(float Stride)
{
	EyePosition += Stride * RightDirection;
	FocusPosition += Stride * RightDirection;
}

void Camera::RotateByY(float AngleY)
{
	XMMATRIX Rotate = XMMatrixRotationAxis(RightDirection, -AngleY);

	UpDirection = XMVector3TransformNormal(UpDirection, Rotate);
	ViewDirection = XMVector3TransformNormal(ViewDirection, Rotate);

	FocusPosition = EyePosition + ViewDirection * FocalLength;
}

void Camera::RotateByX(float AngleX)
{
	XMMATRIX Rotate = XMMatrixRotationY(AngleX);

	UpDirection = XMVector3TransformNormal(UpDirection, Rotate);
	ViewDirection = XMVector3TransformNormal(ViewDirection, Rotate);
	RightDirection = XMVector3TransformNormal(RightDirection, Rotate);
...
void Camera::CameraRotate()
{
	...
	float AngleX = XMConvertToRadians(0.25f * static_cast<float>(CurrentCursorPoint.x - LastCursorPoint.x));
	float AngleY = XMConvertToRadians(0.25f * static_cast<float>(CurrentCursorPoint.y - LastCursorPoint.y));
...
private:
	...
	ComPtr<ID3D12Resource> mCBVResource;
	struct CBuffer
	{
	...
	ComPtr<ID3D12Resource> mIndexResource;
...
{
public:
	// inline static std::function<LRESULT(HWND, UINT, WPARAM, LPARAM)> Broker_Function;

	static LRESULT CALLBACK CallbackFunc(HWND HWnd, UINT Message, WPARAM WParam, LPARAM LParam)
	{
		DX12Engine* pThis = reinterpret_cast<DX12Engine*>(GetWindowLongPtr(HWnd, GWLP_USERDATA));
		if (pThis)
		{
			return pThis->CallBackFunc(HWnd, Message, WParam, LParam);
		}
...
void DX12Engine::InitWindow(HINSTANCE HInstance)
{
    ...
    WindowClassInfo.lpfnWndProc = CallbackWrapper::CallbackFunc;
    WindowClassInfo.lpszClassName = L"006-FP";
...
void DX12Engine::CreateCBVResource()
{
	...
	mD3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE, &CBVResourceDesc,
		D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&mCBVResource));

	mCBVResource->Map(0, nullptr, reinterpret_cast<void**>(&MVPBuffer));
}
...
void DX12Engine::CreateRootSignature()
{
	...
	RootParameters[0].DescriptorTable = RootDescriptorTableDesc;

	D3D12_ROOT_DESCRIPTOR CBVRootDescriptorDesc = {};
	CBVRootDescriptorDesc.ShaderRegister = 0;
	CBVRootDescriptorDesc.RegisterSpace = 0;

	RootParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
	RootParameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	RootParameters[1].Descriptor = CBVRootDescriptorDesc;
...
void DX12Engine::CreatePSO()
{
    ...
    ComPtr<ID3DBlob> ErrorBlob;
    D3DCompileFromFile(L"006-Shader.hlsl", nullptr, nullptr, "VSMain", "vs_5_1", 0, 0, &VertexShaderBlob, &ErrorBlob);
    if (ErrorBlob)
    {
        OutputDebugStringA((const char*)ErrorBlob->GetBufferPointer());
        OutputDebugStringA("\n");
    }
    D3DCompileFromFile(L"006-Shader.hlsl", nullptr, nullptr, "PSMain", "ps_5_1", 0, 0, &PixelShaderBlob, &ErrorBlob);
    if (ErrorBlob)
    {
        OutputDebugStringA((const char*)ErrorBlob->GetBufferPointer());
        OutputDebugStringA("\n");
...
void DX12Engine::CreateIndexResource()
{
	UINT IndexArray[36] =
	{
		0,1,2,0,2,3,
	...
	IndexResourceDesc.Width = sizeof(IndexArray);
	...
	mD3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE,
		&IndexResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&mIndexResource));

	BYTE* TransferPointer = nullptr;
	mIndexResource->Map(0, nullptr, reinterpret_cast<void**>(&TransferPointer));
	memcpy(TransferPointer, IndexArray, sizeof(IndexArray));
	mIndexResource->Unmap(0, nullptr);
...
void DX12Engine::UpdateConstantBuffer()
{
	// ModelMatrix = XMMatrixRotationY(30.0f);
	// ViewMatrix = XMMatrixLookAtLH(EyePosition, FocusPosition, UpDirection);
	// ProjectionMatrix = XMMatrixPerspectiveFovLH(XM_PIDIV4, 4.0 / 3, 0.1, 1000);
	XMStoreFloat4x4(&MVPBuffer->MVPMatrix, mFirstPersonCamera.GetMVPMatrix());
}
...
void DX12Engine::Render()
{
	...
	// CBuffer
	mCommandList->SetGraphicsRootConstantBufferView(1, mCBVResource->GetGPUVirtualAddress());
	...
	case WM_MOUSEMOVE:
		{
			...
			{
			case MK_LBUTTON:
				mFirstPersonCamera.CameraRotate();
				break;
			default:
				mFirstPersonCamera.UpdateCursorPos();
			}
		}
		break;
	default:
